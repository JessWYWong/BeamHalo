...
template <class T>
void BeamHaloAnalyzer_Tree::TypedBranchConnector<T>::
...
template <class T> 
BeamHaloAnalyzer_Tree::TypedBranchConnector<T>::
...
void BeamHaloAnalyzer_Tree::beginJob() {}


BeamHaloAnalyzer_Tree::BeamHaloAnalyzer_Tree(const edm::ParameterSet& iConfig) {
  ...
  edm::ProductSelectorRules productSelectorRules_(iConfig, "outputCommands", "BeamHaloAnalyzer_Tree");
    ...
    if(productSelector_.selected(*selection)) {
      ...
      if (branchnames.find( selection->productInstanceName()) != branchnames.end() ) {
        throw edm::Exception(edm::errors::Configuration)
          ...
          << "Exception thrown from BeamHaloAnalyzer_Tree::BeamHaloAnalyzer" << std::endl;
      }
      else {
        branchnames.insert( selection->productInstanceName() );
      }

      //Create BeamHaloAnalyzer_Tree branch
      ...
      default:
        {
          ...
          throw edm::Exception(edm::errors::Configuration)
            << "class BeamHaloAnalyzer_Tree does not handle leaves of type " << selection->className() << " like\n"
            ...
            <<   leafstring
            << "Exception thrown from BeamHaloAnalyzer_Tree::BeamHaloAnalyzer\n";
        }
      }
    }
  }
