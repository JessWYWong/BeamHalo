#ifndef BEAMHALOANALYZER_HCALDIGIALGORITHM_H
#define BEAMHALOANALYZER_HCALDIGIALGORITHM_H
...
class BeamHaloAnalyzer_HcalDigiAlgorithm { 

  public:

    BeamHaloAnalyzer_HcalDigiAlgorithm();
    ...
    std::unique_ptr<std::vector<float> > rec_chi2;
      ...
      void run ( const HcalDbService    & conditions,  
        ...
        for (; digi != digi_end ; ++digi ) {
          ...
          for (std::vector<std::vector<int> >::iterator it = channelFilterList.begin(); it != channelFilterList.end(); ++it) {
            ...
            //if ((*it).at(2) > 1) filterChannel = false; 
          }
