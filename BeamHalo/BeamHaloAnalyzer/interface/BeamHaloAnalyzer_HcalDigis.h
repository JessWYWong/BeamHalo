#ifndef BeamHaloAnalyzer_HcalDigis_h
#define BeamHaloAnalyzer_HcalDigis_h
...
#include "BeamHalo/BeamHaloAnalyzer/interface/BeamHaloAnalyzer_HcalDigiAlgorithm.h"

template <class DigiCollection, class RecHitCollection, class DetIdClass, class DetIdClassWrapper> 
class BeamHaloAnalyzer_HcalDigis : public edm::EDProducer {
...
protected:
  ...
  BeamHaloAnalyzer_HcalDigiAlgorithm algo;
...
public:
  ...
  BeamHaloAnalyzer_HcalDigis(const edm::ParameterSet& iConfig) :
    ...
    m_channelFilterList (iConfig.getUntrackedParameter<std::vector<edm::ParameterSet> >("ChannelFilterList"))
    {
      ...
      produces<std::vector<float> >               ( m_prefix + "Recchi2"         + m_suffix );
...
protected:
  ...
  void loadAlgo(){
    ...
    algo.rec_chi2        = std::unique_ptr<std::vector<float> >               ( new std::vector<float> ());
  ...
  void dumpAlgo ( edm::Event & iEvent ){
    ...
    iEvent.put( move(algo.rec_chi2        ), m_prefix + "Recchi2"         + m_suffix );
  }
};
...
typedef BeamHaloAnalyzer_HcalDigis<HBHEDigiCollection     , HBHERecHitCollection, HcalDetId     , HcalDetId            > BeamHaloAnalyzer_HBHEDigis;
//typedef BeamHaloAnalyzer_HcalDigis<HODigiCollection       , HORecHitCollection  , HcalDetId     , HcalDetId            > BeamHaloAnalyzer_HODigis;
//typedef BeamHaloAnalyzer_HcalDigis<HFDigiCollection       , HFRecHitCollection  , HcalDetId     , HcalDetId            > BeamHaloAnalyzer_HFDigis;
//typedef BeamHaloAnalyzer_HcalDigis<HcalCalibDigiCollection, HBHERecHitCollection, HcalCalibDetId, HcalCalibDetIdWrapper> BeamHaloAnalyzer_CalibDigis;
